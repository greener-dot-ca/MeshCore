...
public:
  ...
  // copy `src` -> `dest`, keeping printable ASCII and valid UTF-8 sequences and
  // replacing control / invalid bytes with '?' (so untrusted text can't render
  // as garbage glyphs). Always NUL-terminates.
  static void sanitizeText(char* dest, const char* src, size_t buf_sz);
